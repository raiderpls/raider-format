...
s.listed_series = {0xe01, 0x1e01}
s.listed_names = {id}

function s.filter(c,e,tp)
	return c:IsSetCard(0x1e01) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false) and c:IsAttribute(e:GetHandler():GetAttribute())
end
...
function s.thfilter(c)
	return c:IsSetCard(0xe01) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local group = Duel.SelectMatchingCard(tp, s.thfilter, tp, LOCATION_GRAVE, 0, 1, 1, nil)
	if #group > 0 then
